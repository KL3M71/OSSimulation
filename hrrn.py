...
def hrrn(processes):
    ...
    # Helper function to find the process with the highest response ratio
    def find_highest_response_ratio(ready_queue):
        return max(ready_queue, key=lambda x: processes[x].response_ratio(current_time))
    ...
    while sum([p.remaining_time for p in processes]) > 0 or ready_queue or io_queue:
        ...
        if ready_queue:
            current_process = find_highest_response_ratio(ready_queue)
            ...
            current_time += 1  # If there are no processes in the ready queue, increment the current time
    ...
    with open('hrrn_results.csv', 'w', newline='') as csvfile:
